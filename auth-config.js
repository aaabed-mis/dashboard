...
/* Sessions live in localStorage, which is per-ORIGIN. Production is ONE origin
   (dashboard.abederp.com): the hub at "/", each dashboard at "/<id>/", so the
   session is already visible there — no handoff needed. The cross-origin path
...
function resolveHubUrl() {
  ...
  return new URL('./', location.href).href;    // one local server: the hub IS the root
}
