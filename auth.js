/* Hub login gate — the ONLY place a password is entered.
   ...
   The suite is one origin (dashboard.abederp.com): the hub at "/", each
   dashboard at "/<id>/", so the session this login creates is already visible
   to every dashboard — no handoff in production. The fragment hand-off in
   app.js only matters for the offline four-port cross-origin harness.
